...
_面板支援 6 種語言 —— 用上方語言切換可看到對應語言的截圖。_

**11 種入站協議一覽：流行度、是否需要網域、CDN / Argo 支援與網路適應性一目了然。**

![協議指南](docs/screenshots/zh-TW/protocols.jpg)

**勾選你要用的用戶端 App，EdgeNest 為每個入站客製並產生可直接匯入的設定。**

![用戶端選擇](docs/screenshots/zh-TW/clients.jpg)

**可選 CDN 前置：用戶端經 Cloudflare 優選 IP 接入更快，真實伺服器 IP 不外露。**

![CDN 前置](docs/screenshots/zh-TW/cdn.jpg)

**可選 Argo 隧道：用戶端經 Cloudflare 隧道接入，無需暴露伺服器 IP、無需開埠。**

![Argo 隧道](docs/screenshots/zh-TW/argo.jpg)
...
**環境需求**：一台全新的 64 位 Linux VPS（Debian / Ubuntu 等，見下方「支援的平台」），具備 root 權限、可用的套件管理器與連網。安裝指令碼會自動裝好所需相依套件（curl、git、sqlite3、iptables 等），並優先使用預編譯二進位檔——所以 **1 核 / 1 GB（甚至 512 MB）的 VPS 也無需現場編譯即可安裝**。極簡映像若連 `curl` 甚至 `sudo` 都沒有，直接用 `root` 跑安裝指令碼即可，它會自行補齊。
...
安裝後，隨時在伺服器上執行 **`edgenest`** 即可叫出管理選單——查看面板位址與管理員帳號、重啟 / 停止 / 啟動服務、查看即時日誌、重設管理員密碼、升級到最新穩定版、解除安裝。如果忘了把面板位址加入書籤，這是最快找回的方式。
...
| `EDGENEST_VERSION` | `1.20.0626` | 預編譯產物下載版本 |
