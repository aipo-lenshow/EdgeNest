...
**Создание через мастер: выберите сценарий — EdgeNest заранее подберёт набор протоколов и перейдёт к быстрому созданию.**

![Создание входящего по сценарию](docs/screenshots/01-create-inbound-scenarios.jpg)

**Все 11 входящих протоколов с одного взгляда: популярность, нужен ли домен, поддержка CDN / Argo и устойчивость к сети.**

![Справочник протоколов](docs/screenshots/02-protocol-guide.jpg)

**Маршрутизация по категориям в один клик: направляйте трафик по категории сервисов (ИИ / стриминг / инструменты разработчика и т. д.) на нужный исходящий.**

![Правила маршрутизации](docs/screenshots/03-routing-rules.jpg)

**Подписки под каждый клиент: выберите клиенты, из которых будете подписываться, — EdgeNest сгенерирует каждую в её собственном формате, подключение при импорте.**

![Выбор клиентов](docs/screenshots/04-clients.jpg)
...
| `EDGENEST_VERSION` | `1.12.0624` | Версия для скачивания готового артефакта |
