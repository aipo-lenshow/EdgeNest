...
**向导式创建：按使用场景推荐协议组合，预选后进入快速创建。**

![按场景创建入站](docs/screenshots/01-create-inbound-scenarios.jpg)

**11 种入站协议一览：流行度、是否需要域名、CDN / Argo 支持与网络适应性一目了然。**

![协议指南](docs/screenshots/02-protocol-guide.jpg)

**一键分类路由：按服务类别（AI / 流媒体 / 开发工具 等）设置出站走向。**

![分类路由](docs/screenshots/03-routing-rules.jpg)

**按客户端生成订阅：选择你要订阅的客户端，按各客户端自有格式生成，导入即连。**

![客户端选择](docs/screenshots/04-clients.jpg)
...
| `EDGENEST_VERSION` | `1.12.0624` | 预编译产物下载版本 |
