// Small inline SVG icons reused across the panel. Inlined (not <img src>) so they
// inherit currentColor and don't depend on the panel-path base for asset URLs.

// BrandLogo mirrors public/favicon.svg — a double-hex "nest" with a central node
// and three outgoing edges to peer-node rings.
export function BrandLogo({ className = "h-6 w-6" }: { className?: string }) {
  return (
    <svg viewBox="0 0 64 64" className={className} aria-hidden="true">
      <rect width="64" height="64" rx="14" fill="#0b0e14" />
      <g
        fill="none"
        stroke="#10b981"
        strokeWidth="2.6"
        strokeLinejoin="round"
        strokeLinecap="round"
      >
        <polygon points="32,13 50,23 50,41 32,51 14,41 14,23" />
        <polygon points="32,22 42,27.5 42,36.5 32,42 22,36.5 22,27.5" />
        <line x1="50" y1="23" x2="55" y2="18" />
        <line x1="14" y1="23" x2="9" y2="18" />
        <line x1="32" y1="51" x2="32" y2="57" />
        <circle cx="56.5" cy="16.5" r="3.2" />
        <circle cx="7.5" cy="16.5" r="3.2" />
        <circle cx="32" cy="59" r="3.2" />
      </g>
      <circle cx="32" cy="32" r="5" fill="#34d399" />
    </svg>
  );
}

// GithubIcon — the official GitHub mark, tinted via currentColor.
export function GithubIcon({ className = "h-5 w-5" }: { className?: string }) {
  return (
    <svg viewBox="0 0 16 16" className={className} fill="currentColor" aria-hidden="true">
      <path d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.03.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27.68 0 1.36.09 2 .27 1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.01 8.01 0 0016 8c0-4.42-3.58-8-8-8z" />
    </svg>
  );
}
